...
import { InitTimeMap, NodeScheduleTimeMap, RecurrenceRule, ScheduleMap } from '../types/Scheduler.js';


export const generateCycleMapForProcessors = (scheduleMap: ScheduleMap): { [task: string]: NodeScheduleTimeMap } => {
  return transform(
      Object.keys(scheduleMap),
      (acc, task) => acc[task] = generateRecurrenceRuleMapForProcessor(scheduleMap[task].timeMap),
    {} as { [task: string]: NodeScheduleTimeMap }
  );
};
