...
export class ModelRoute<T extends keyof ModelEndpoints, V extends ModelRequest<T>> extends Route<T, V> {
  ...
  constructor(basePath: string, routePath: string, private modelProvider: ModelProvider) {
    ...
    this.subPaths = [
      { path: modelRouteMapping.model.subPaths.generate.name, authenticate: true, handler: this.generate.bind(this) },
      { path: modelRouteMapping.model.subPaths.view.name, authenticate: true, handler: this.view.bind(this) },
      { path: modelRouteMapping.model.subPaths.update.name, authenticate: true, handler: this.update.bind(this) },
      { path: modelRouteMapping.model.subPaths.delete.name, authenticate: true, handler: this.delete.bind(this) }
    ];
  }
